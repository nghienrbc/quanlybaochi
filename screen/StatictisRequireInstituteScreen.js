...
function StatictisRequireInstituteScreen(props) {
    ...
    const onShowChartTotal = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisRequireFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
    ...
    const onShowChartOfPressAgency = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisPressAgencyRequireFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
            .then(responseStatictis => {
                debugger
    ...
    const onShowChartOfJournalist = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisJournalistRequireFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
            .then(responseStatictis => {
                debugger
    ...
    useEffect(() => {
        ...
        if (selectedPressAgencyItems.length == 0) {
            statictisPressAgency.length > 0 && statictisPressAgency.forEach(element => {
                repliedArray.push({ 'x': element.dvpv, 'y': parseInt(element.replied) > 0 ? parseInt(element.replied) : null, 'label': parseInt(element.replied) > 0 ? element.replied : null })
                awaitReplyArray.push({ 'x': element.dvpv, 'y': parseInt(element.awaitReply) > 0 ? parseInt(element.awaitReply) : null, 'label': parseInt(element.awaitReply) > 0 ? element.awaitReply : null })
                cancelReplyArray.push({ 'x': element.dvpv, 'y': parseInt(element.cancelReply) > 0 ? parseInt(element.cancelReply) : null, 'label': parseInt(element.cancelReply) > 0 ? element.cancelReply : null })
            })
    ...
    useEffect(() => {
        ...
        if (selectedJournalistItems.length == 0) {
            statictisJournalist.length > 0 && statictisJournalist.forEach(element => {
                repliedArray.push({ 'x': element.claimSender, 'y': parseInt(element.replied) > 0 ? parseInt(element.replied) : null, 'label': parseInt(element.replied) > 0 ? element.replied : null })
                awaitReplyArray.push({ 'x': element.claimSender, 'y': parseInt(element.awaitReply) > 0 ? parseInt(element.awaitReply) : null, 'label': parseInt(element.awaitReply) > 0 ? element.awaitReply : null })
                cancelReplyArray.push({ 'x': element.claimSender, 'y': parseInt(element.cancelReply) > 0 ? parseInt(element.cancelReply) : null, 'label': parseInt(element.cancelReply) > 0 ? element.cancelReply : null })
            })
            statictisJournalist.length > 0 && setJournalistData({ 'replied': repliedArray, 'awaitReply': awaitReplyArray, 'cancelReply': cancelReplyArray })
        }
        else {
            let hasResult = false
            ...
            statictisJournalist.length > 0 && hasResult && setJournalistData({ 'replied': repliedArray, 'awaitReply': awaitReplyArray, 'cancelReply': cancelReplyArray })
        }
    }, [statictisJournalist])
...
export default StatictisRequireInstituteScreen;
