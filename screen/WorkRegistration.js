...
function WorkRegistration(props) {
  ...
  const callPostWorkRegistration = () => {
    if (tokenString != '') {
      ...
      setShowLoading(true)
      debugger
      workRepo.postWorkRegistration(tokenString, selectedInstituteID, sessionID, title, content, timeStamp, false, false, false, files, 0)
        .then(
          responseWork => {
  ...
  return (
    <SafeAreaView style={styles.saveAreaViewContainer}>
      ...
      <TouchableOpacity onPress={() => {
        showAlert({
          title: "Thông báo",
          message: "Bạn muốn gửi đăng ký làm việc này ?",
          alertType: 'warning',
          btnLabel: 'Đồng ý',
          leftBtnLabel: 'Hủy',
          onPress: () => {
