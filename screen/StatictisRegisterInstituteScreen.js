...
import {
    VictoryPie,
    VictoryAxis, VictoryBar, VictoryChart, VictoryGroup, VictoryLabel
} from 'victory-native';
...
function StatictisRegisterInstituteScreen(props) {
    ...
    const [statictisJournalist, setStatictisJournalist] = useState([])
    ...
    const onShowChartTotal = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisInstituteFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
    ...
    const onShowChartOfPressAgency = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisPressAgencyFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
            .then(responseStatictis => {
                debugger
    ...
    const onShowChartOfJournalist = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisJounalistFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
            .then(responseStatictis => {
                debugger
    ...
    useEffect(() => {
        ...
        statictisTotal.length > 0 && statictisTotal.forEach(element => {
            // kiểm tra nếu session đó phóng viên đã join vào thì không đưa vào danh sách nữa  
            let statictisObj = {
                "time": element.time,
                "total": element.data.total,
                "cancel": element.data.cancel,
                "agree": element.data.agree,
                "wait2": element.data.wait2,
                "wait3": element.data.wait3,
                "finish": element.data.finish
            }
            categoriesData[0].total += parseInt(element.data.finish)
            categoriesData[1].total += parseInt(element.data.wait2)
            categoriesData[2].total += parseInt(element.data.wait3)
            categoriesData[3].total += parseInt(element.data.agree)
            categoriesData[4].total += parseInt(element.data.cancel)
        })
    }, [statictisTotal])
    ...
    useEffect(() => {
        ...
        if (selectedPressAgencyItems.length == 0) {
            statictisPressAgency.length > 0 && statictisPressAgency.forEach(element => {
                wait2Array.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.wait2) > 0 ? parseInt(element.wait2) : null, 'label': parseInt(element.wait2) > 0 ? element.wait2 : null })
                wait3Array.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.wait3) > 0 ? parseInt(element.wait3) : null, 'label': parseInt(element.wait3) > 0 ? element.wait3 : null })
                agreeArray.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.gree) > 0 ? parseInt(element.gree) : null, 'label': parseInt(element.gree) > 0 ? element.gree : null })
                cancelArray.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.cancel) > 0 ? parseInt(element.cancel) : null, 'label': parseInt(element.cancel) > 0 ? element.cancel : null })
                finishArray.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.finish) > 0 ? parseInt(element.finish) : null, 'label': parseInt(element.finish) > 0 ? element.finish : null })
            })
            statictisPressAgency.length > 0 && setPressAgencyData({ 'wait2': wait2Array, 'wait3': wait3Array, 'agree': agreeArray, 'cancel': cancelArray, 'finish': finishArray })
        }
        else {
            let hasResult = false
            selectedPressAgencyItems.forEach(element1 => {
                statictisPressAgency.length > 0 && statictisPressAgency.forEach(element => {
                    if (element1 == element.senderUsers.instituteId) {
                        wait2Array.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.wait2) > 0 ? parseInt(element.wait2) : null, 'label': parseInt(element.wait2) > 0 ? element.wait2 : null })
                        wait3Array.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.wait3) > 0 ? parseInt(element.wait3) : null, 'label': parseInt(element.wait3) > 0 ? element.wait3 : null })
                        agreeArray.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.gree) > 0 ? parseInt(element.gree) : null, 'label': parseInt(element.gree) > 0 ? element.gree : null })
                        cancelArray.push({ 'x': element.senderUsers?.Institute?.name, 'y': parseInt(element.cancel) > 0 ? parseInt(element.cancel) : null, 'label': parseInt(element.cancel) > 0 ? element.cancel : null })
            ...
            statictisPressAgency.length > 0 && hasResult && setPressAgencyData({ 'wait2': wait2Array, 'wait3': wait3Array, 'agree': agreeArray, 'cancel': cancelArray, 'finish': finishArray })
        }
    ...
    useEffect(() => {
        ...
        if (selectedJournalistItems.length == 0) {
            statictisJournalist.length > 0 && statictisJournalist.forEach(element => {
                wait2Array.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.wait2) > 0 ? parseInt(element.wait2) : null, 'label': parseInt(element.wait2) > 0 ? element.wait2 : null })
                wait3Array.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.wait3) > 0 ? parseInt(element.wait3) : null, 'label': parseInt(element.wait3) > 0 ? element.wait3 : null })
                agreeArray.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.gree) > 0 ? parseInt(element.gree) : null, 'label': parseInt(element.gree) > 0 ? element.gree : null })
                cancelArray.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.cancel) > 0 ? parseInt(element.cancel) : null, 'label': parseInt(element.cancel) > 0 ? element.cancel : null })
                finishArray.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.finish) > 0 ? parseInt(element.finish) : null, 'label': parseInt(element.finish) > 0 ? element.finish : null })
            })
            statictisJournalist.length > 0 && setJournalistData({ 'wait2': wait2Array, 'wait3': wait3Array, 'agree': agreeArray, 'cancel': cancelArray, 'finish': finishArray })
        }
        else {
            let hasResult = false
            selectedJournalistItems.forEach(element1 => {
                statictisJournalist.length > 0 && statictisJournalist.forEach(element => {
                    if (element1 == element.senderId) {
                        wait2Array.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.wait2) > 0 ? parseInt(element.wait2) : null, 'label': parseInt(element.wait2) > 0 ? element.wait2 : null })
                        wait3Array.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.wait3) > 0 ? parseInt(element.wait3) : null, 'label': parseInt(element.wait3) > 0 ? element.wait3 : null })
                        agreeArray.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.gree) > 0 ? parseInt(element.gree) : null, 'label': parseInt(element.gree) > 0 ? element.gree : null })
                        cancelArray.push({ 'x': element.senderUsers.givenName, 'y': parseInt(element.cancel) > 0 ? parseInt(element.cancel) : null, 'label': parseInt(element.cancel) > 0 ? element.cancel : null })
            ...
            statictisJournalist.length > 0 && hasResult && setJournalistData({ 'wait2': wait2Array, 'wait3': wait3Array, 'agree': agreeArray, 'cancel': cancelArray, 'finish': finishArray })
        }
    }, [statictisJournalist])
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 10, backgroundColor: COLORS.background }}>
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{ flexDirection: 'row', padding: 3 }}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => ExportToExcel.handleExportPress(pressAgencyData, ExportToExcel.ExportTypeEnum.DKLV, ExportToExcel.ExportRoleEnum.BC, 'DS_DKLV_ThuongTru')
                            }>
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{ flexDirection: 'row', padding: 3 }}>
                        ...
                        <TouchableOpacity
                            style={[styles.touchOpacity, { width: '40%' }]}
                            onPress={() => ExportToExcel.handleExportPress(journalistData, ExportToExcel.ExportTypeEnum.DKLV, ExportToExcel.ExportRoleEnum.PV, 'DS_DKLV_ThuongTru')
                            }>
                            <Export name='file-export' style={{ color: 'white', fontSize: 15, marginRight: 10 }} />
                            <Text style={{ fontSize: 13, color: 'white', fontWeight: 'bold' }}> Xuất báo cáo</Text>
                        </TouchableOpacity>
...
export default StatictisRegisterInstituteScreen;
