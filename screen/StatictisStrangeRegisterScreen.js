...
function StatictisStrangeRegisterScreen(props) {
    ...
    useEffect(() => {
        debugger
        let statictisName = 'Thống kê đăng ký vãng lai'
    ...
    const onShowChartTotal = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisStrangeRegisterFromInstitute(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
    ...
    const onShowChartOfJournalist = () => {
        ...
        setShowLoading(true)
        statictisRepo.getStatictisStrangeRegisterFromJournalist(tokenString, new Date(fromDate).getTime(), new Date(toDate).getTime())
            .then(responseStatictis => {
                // debugger
    ...
    useEffect(() => {
        ...
        setStatictisTotal.length > 0 && statictisTotal.forEach(element => {
            ...
            //debugger
            categoriesData[0].total += parseInt(element.agree)
            categoriesData[1].total += parseInt(element.finish)
            categoriesData[2].total += parseInt(element.cancelfinish)
        })
    }, [statictisTotal])
    ...
    useEffect(() => {
        ...
        if (selectedPressAgencyItems.length == 0) {
            statictisPressAgency.length > 0 && statictisPressAgency.forEach(element => {
                agreeArray.push({ 'x': element.namedvbc, 'y': parseInt(element.agree) > 0 ? parseInt(element.agree) : null, 'label': parseInt(element.agree) > 0 ? element.agree : null })
                finishArray.push({ 'x': element.namedvbc, 'y': parseInt(element.finish) > 0 ? parseInt(element.finish) : null, 'label': parseInt(element.finish) > 0 ? element.finish : null })
                cancelArray.push({ 'x': element.namedvbc, 'y': parseInt(element.cancelfinish) > 0 ? parseInt(element.cancelfinish) : null, 'label': parseInt(element.cancelfinish) > 0 ? element.cancelfinish : null })
            })
            statictisPressAgency.length > 0 && setPressAgencyData({ 'agree': agreeArray, 'finish': finishArray, 'cancel': cancelArray })
        }
        else {
            let hasResult = false
            selectedPressAgencyItems.forEach(element1 => {
                statictisPressAgency.length > 0 && statictisPressAgency.forEach(element => {
                    if (element1 == element.iddvbc) {
                        agreeArray.push({ 'x': element.namedvbc, 'y': parseInt(element.agree) > 0 ? parseInt(element.agree) : null, 'label': parseInt(element.agree) > 0 ? element.agree : null })
                        finishArray.push({ 'x': element.namedvbc, 'y': parseInt(element.finish) > 0 ? parseInt(element.finish) : null, 'label': parseInt(element.finish) > 0 ? element.finish : null })
                        cancelArray.push({ 'x': element.namedvbc, 'y': parseInt(element.cancelfinish) > 0 ? parseInt(element.cancelfinish) : null, 'label': parseInt(element.cancelfinish) > 0 ? element.cancelfinish : null })
                        hasResult = true
            ...
            statictisPressAgency.length > 0 && hasResult && setPressAgencyData({ 'agree': agreeArray, 'finish': finishArray, 'cancel': cancelArray })
        }
    ...
    const StatisticColumn = () => {
        return <View style={{ backgroundColor: 'white' }}>
            ...
            <ScrollView horizontal={true}>
                <View style={{ width: pressAgencyData.agree.length * 160 > SIZES.width ? pressAgencyData.agree.length * 160 : SIZES.width - 40, borderRadius: 20 }}>
                    <VictoryChart
                        domainPadding={60}
                        width={pressAgencyData.agree.length * 160 > SIZES.width ? pressAgencyData.agree.length * 160 : SIZES.width - 40}
                        height={300}
                        style={{
                            parent: {
                    ...
                    >
                        ...
                        />
                        <VictoryGroup
                            offset={pressAgencyData.agree.length > 1 ? 20 : 20}
                            style={{ marginLeft: 0 }}>
    ...
    useEffect(() => {
        ...
        if (selectedJournalistItems.length == 0) {
            statictisJournalist.length > 0 && statictisJournalist.forEach(element => {
                agreeArray.push({ 'x': element.usersender, 'y': parseInt(element.agree) > 0 ? parseInt(element.agree) : null, 'label': parseInt(element.agree) > 0 ? element.agree : null })
                finishArray.push({ 'x': element.usersender, 'y': parseInt(element.finish) > 0 ? parseInt(element.finish) : null, 'label': parseInt(element.finish) > 0 ? element.finish : null })
                cancelArray.push({ 'x': element.usersender, 'y': parseInt(element.cancelfinish) > 0 ? parseInt(element.cancelfinish) : null, 'label': parseInt(element.cancelfinish) > 0 ? element.cancelfinish : null })
            })
            statictisJournalist.length > 0 && setJournalistData({ 'agree': agreeArray, 'finish': finishArray, 'cancel': cancelArray })
        }
        else {
            let hasResult = false
            ...
            statictisJournalist.length > 0 && hasResult && setJournalistData({ 'agree': agreeArray, 'finish': finishArray, 'cancel': cancelArray })
        }
    ...
    const JournalistStatisticColumn = () => {
        return <View style={{ backgroundColor: 'white' }}>
            ...
            <ScrollView horizontal={true}>
                <View style={{ width: journalistData.agree.length * 160 > SIZES.width ? journalistData.agree.length * 160 : SIZES.width - 40, borderRadius: 20 }}>
                    <VictoryChart
                        domainPadding={50}
                        width={journalistData.agree.length * 160 > SIZES.width ? journalistData.agree.length * 160 : SIZES.width - 40}
                        height={300}
                        style={{
                            parent: {
                                backgroundColor: "white",
                    ...
                    >
                        ...
                        />
                        <VictoryGroup
                            offset={journalistData.agree.length > 1 ? 12 : 20}
                            style={{ marginLeft: 0 }}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 10, backgroundColor: COLORS.background }}>
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{ flexDirection: 'row', padding: 3 }}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => ExportToExcel.handleExportPress(pressAgencyData, ExportToExcel.ExportTypeEnum.DKLV, ExportToExcel.ExportRoleEnum.BC, 'DS_DKLV_VangLai')
                            }>
                ...
                <View style={{ padding: 20 }}>
                    ...
                    <View style={{ flexDirection: 'row', padding: 3 }}>
                        ...
                        <TouchableOpacity
                            style={[styles.touchOpacity, { width: '40%' }]}
                            onPress={() => ExportToExcel.handleExportPress(journalistData, ExportToExcel.ExportTypeEnum.DKLV, ExportToExcel.ExportRoleEnum.PV, 'DS_DKLV_VangLai')
                            }>
                            <Export name='file-export' style={{ color: 'white', fontSize: 15, marginRight: 10 }} />
                            <Text style={{ fontSize: 13, color: 'white', fontWeight: 'bold' }}> Xuất báo cáo</Text>
                        </TouchableOpacity>
...
export default StatictisStrangeRegisterScreen;
